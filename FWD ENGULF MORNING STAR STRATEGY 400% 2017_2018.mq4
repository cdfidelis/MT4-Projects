...
extern int TakeProfit=450;
extern int StopOut_Strategy1=450;
extern int StopOut_Strategy2=400;
extern double lot = 0.50;



double TrailingStop  =0;
int No_of_Candles = 10000; 
int peri_d = 50;
...
//(iTime(tradpairs,PERIOD_H1,1)-iTime(tradpairs,PERIOD_H1,2)==iTime(tradpairs,PERIOD_H1,3)-iTime(tradpairs,PERIOD_H1,4))

void OnTick()
{
if(Symbol()!="EURUSD"){Print("Change Currency Pair to EURUSD, Timeframe:60mins");}

if(Symbol()=="EURUSD" 
//&& TimeHour(TimeCurrent())>=4 && TimeHour(TimeCurrent())<=21
...
{
     if((iHigh(tradpairs, PERIOD_D1, 0)==iHigh(tradpairs, PERIOD_H1, 3)||
         iHigh(tradpairs, PERIOD_D1, 0)==iHigh(tradpairs, PERIOD_H1, 2)||
         iHigh(tradpairs, PERIOD_D1, 0)==iHigh(tradpairs, PERIOD_H1, 1))&&
         
      iHigh(tradpairs, PERIOD_D1, 0)-iHigh(tradpairs, PERIOD_D1, 1) > altitude*MarketInfo(Symbol(),MODE_POINT)&&
      
      //   (iClose(tradpairs, PERIOD_D1, 0) - iOpen(tradpairs, PERIOD_D1, 1) > 400*MarketInfo(Symbol(),MODE_POINT)||
      //  iClose(tradpairs, PERIOD_D1, 0) - iOpen(tradpairs, PERIOD_D1, 1) > 400*MarketInfo(Symbol(),MODE_POINT))&&
      //iHigh(tradpairs, PERIOD_D1, 0)-iLow(tradpairs, PERIOD_D1, 4)> 1000*MarketInfo(Symbol(),MODE_POINT)&&
         
     /*((iClose(tradpairs,0, i+4) - iOpen(tradpairs,0, i+4)>10*MarketInfo(Symbol(),MODE_POINT)&&
     iClose(tradpairs,0, i+3) - iOpen(tradpairs,0, i+3)>10*MarketInfo(Symbol(),MODE_POINT))|| 
     iClose(tradpairs,0, i+2) - iOpen(tradpairs,0, i+2)>150*MarketInfo(Symbol(),MODE_POINT)||
     iClose(tradpairs,0, i+3) - iOpen(tradpairs,0, i+3)>150*MarketInfo(Symbol(),MODE_POINT))
     &&*/
     
     (iClose(tradpairs,0, i+2) - iOpen(tradpairs,0, i+2)> 80*MarketInfo(Symbol(),MODE_POINT)||
     (iClose(tradpairs,0, i+4) - iOpen(tradpairs,0, i+4)>10*MarketInfo(Symbol(),MODE_POINT)&&
     iClose(tradpairs,0, i+3) - iOpen(tradpairs,0, i+3)>10*MarketInfo(Symbol(),MODE_POINT)&&
     iClose(tradpairs,0, i+2) - iOpen(tradpairs,0, i+2)>10*MarketInfo(Symbol(),MODE_POINT)&&
     ...
     iOpen(tradpairs,0, i+1) - iClose(tradpairs,0, i+1) <= 80*MarketInfo(Symbol(),MODE_POINT))||
     (iClose(tradpairs,0, i+1) - iOpen(tradpairs,0, i+1) > 0*MarketInfo(Symbol(),MODE_POINT)&&
     iClose(tradpairs,0, i+1) - iOpen(tradpairs,0, i+1) <=80*MarketInfo(Symbol(),MODE_POINT))
     )
     
     &&iOpen(tradpairs,0, i)- iClose(tradpairs,0, i)>=10*MarketInfo(Symbol(),MODE_POINT)
     
    // &&iHigh(tradpairs, PERIOD_D1, 0)-iClose(tradpairs, PERIOD_H1,i)>0*MarketInfo(tradpairs,MODE_POINT)
    // &&iHigh(tradpairs, PERIOD_D1, 0)-iClose(tradpairs, PERIOD_H1,i)<450*MarketInfo(tradpairs,MODE_POINT)
     
   //  &&iHigh(tradpairs,0, i+1)>=iHigh(tradpairs,0, i)
   //  &&iHigh(tradpairs,0, i+1)>=iHigh(tradpairs,0, i+2)
       
     &&(allowMst==1 || allowMst2==1)
     
        
     )
    { st_S =   iHigh(tradpairs,0, i+2);

placesellord(StopOut_Strategy2,TakeProfit); 
...
if( ((iLow(tradpairs, PERIOD_D1, 0)==iLow(tradpairs, PERIOD_H1, 1)
   ...
   &&iLow(tradpairs, PERIOD_D1, 1)-iLow(tradpairs, PERIOD_D1, 0) > altitude*MarketInfo(Symbol(),MODE_POINT) 
 ...
 // (iLow(tradpairs, PERIOD_H1, 1)-iLow(tradpairs, PERIOD_D1, 0)<250*MarketInfo(tradpairs,MODE_POINT)&&h_Fwd_engulf_pat_B(0,1)==1
  ...
  iHigh(tradpairs, 0, 3)- iLow(tradpairs, 0, 3)<150*MarketInfo(tradpairs,MODE_POINT)
   ...
   h_placebuyord(StopOut_Strategy1,TakeProfit);
...
{     if((iLow(tradpairs, PERIOD_D1, 0)==iLow(tradpairs, PERIOD_H1, 1)||
   ...
   placebuyord(StopOut_Strategy2,TakeProfit);
...
if(((iHigh(tradpairs, PERIOD_D1, 0)==iHigh(tradpairs, PERIOD_H1, 1 )
 ...
 // (iHigh(tradpairs, PERIOD_D1, 0)-iHigh(tradpairs, PERIOD_H1, 1)<250*MarketInfo(tradpairs,MODE_POINT)&& h_Fwd_engulf_pat_S(0,1)==1
  ...
  //&& allow4==1 //&&allow5 ==1
   ...
   h_placesellord(StopOut_Strategy1,TakeProfit);
